...
export default {
    created(){
        ...
        //lecturers
        this.$http.get('http://bcas.localhost/api/admin/getlecturer')
        .then((data)=>{
    ...
    watch:{
       ...
       lecturer:function(){
           if(this.lecturer!=='Lecturer'){
                  this.batches=[];
                this.$http.post('http://bcas.localhost/api/admin/getbatches',{
    ...
    methods:{
        bookhall:function(){
            ...
            if(!this.time || !this.lecturer || !this.batch || !this.month || !this.date){
                ...
                this.load=true;
                var date=new Date();
                console.log(this.$route.params.floornum,this.$route.params.roomnum);
                this.$http.post('http://bcas.localhost/api/admin/bookhall',{
                    time:this.time,
                    lecturer:this.lecturer,
                    department:this.batch.split(' ')[0],
                    batchid:this.batch.split(' ')[this.batch.split(' ').length-1],
        ...
        edithall:function(){
            ...
            if(!this.time || !this.lecturer || !this.batch || !this.month || !this.date){
                ...
                var date=new Date();
                  console.log(this.$route.params.floornum,this.$route.params.roomnum);
                this.$http.post('http://bcas.localhost/api/admin/edithall',{
                    time:this.time,
                    lecturer:this.lecturer,
                    department:this.batch.split(' ')[0],
                     batchid:this.batch.split(' ')[this.batch.split(' ').length-1],
